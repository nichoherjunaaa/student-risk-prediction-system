              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-red-50 border border-red-200 rounded-xl p-6 shadow-sm">
                  <h3 className="text-lg font-bold text-red-800 mb-2 flex items-center">
                    <ShieldAlert className="h-5 w-5 mr-2" /> Belum Lulus ({sks_failed} SKS)
                  </h3>
                      ...
                      {failed_subjects.map((sub, idx) => (
                        <li key={idx} className="flex items-start bg-white p-3 rounded-lg border border-red-100">
                          ...
                          <div>
                             ...
                             <span className="block text-xs text-red-700">Nilai: {sub.nilai} | {sub.sks} SKS</span>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-green-700 font-medium mt-4">Tidak ada riwayat nilai tidak lulus.</p>
                  )}
                </div>

                <div className="bg-green-50 border border-green-200 rounded-xl p-6 shadow-sm">
                  <h3 className="text-lg font-bold text-green-800 mb-2 flex items-center">
                    <BookOpen className="h-5 w-5 mr-2" /> Telah Lulus ({sks_passed} SKS)
                  </h3>
                  {passed_subjects.length > 0 ? (
                    <ul className="space-y-3 mt-4 max-h-60 overflow-y-auto pr-2">
                      {passed_subjects.map((sub, idx) => (
                        <li key={idx} className="flex items-start bg-white p-2 rounded-lg border border-green-100">
                          <div className="w-full flex justify-between items-center">
                             <span className="text-sm text-green-900 font-bold">{sub.matkul}</span>
                             <span className="text-xs text-green-700 font-semibold bg-green-100 px-2 py-1 rounded">Nilai: {sub.nilai}</span>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-gray-500 font-medium mt-4">Belum ada data nilai lulus.</p>
                  )}
                </div>
              </div>
